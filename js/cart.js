...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const AuricCart = {
    ...
    startPeriodicSync: function(userId) {
      ...
      // Instead of continuous polling, we'll only sync when there are actual changes
     ...
     * SIMPLIFIED for better reliability and to prevent cart sync issues
     */
    setupFirestoreListener: function(userId) {
      ...
      // Set up a new listener with better error handling
      try {
        this.firestoreUnsubscribe = db.collection('carts').doc(userId)
          .onSnapshot({
            includeMetadataChanges: true
          }, doc => {
            // Double-check for order completion
            const orderCompleted = localStorage.getItem('orderCompleted') === 'true';
            ...
            // Skip updates when we're already updating Firestore to prevent sync loops
            if (this.isUpdatingFirestore) {
              console.log('Already updating Firestore - skipping this update');
              return;
            }
            ...
            if (doc.exists) {
              ...
              if (serverData && serverData.items) {
                ...
                // Don't update if cart is empty in Firestore but we have items locally
                if (serverItemCount === 0 && this.items.length > 0) {
                  console.log('Keeping local cart items instead of empty server cart');
                  // Instead sync our items to the server
                  setTimeout(() => {
                    this.saveCartToFirestore(userId);
                  }, 1000);
                  return;
